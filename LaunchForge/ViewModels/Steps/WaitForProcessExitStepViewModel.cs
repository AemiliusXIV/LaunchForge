...
public partial class WaitForProcessExitStepViewModel : StepViewModelBase
{
    [ObservableProperty]
    ...
    public override string Summary => string.IsNullOrEmpty(ProcessName)
        ? "(no process)"
        : TimeoutSeconds > 0 ? $"{ProcessName} exits (max {TimeoutSeconds}s)" : $"{ProcessName} exits (∞)";

    public override string TypeTag => "WAIT↓";

    public override StepBase ToModel() => new WaitForProcessExitStep
    {
        Label          = Label,
        IsEnabled      = IsEnabled,
    ...
    {
        var m = (WaitForProcessExitStep)model;
        Label          = m.Label;
